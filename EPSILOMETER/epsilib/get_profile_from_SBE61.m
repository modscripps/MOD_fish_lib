...
nb_downcast=0;
for i=1:max(PressureTimeseries.section_number)
    nb_downcast=nb_downcast+1;
    idx_downcast=find(PressureTimeseries.section_number==i);
    PressureTimeseries.startprof(nb_downcast)=idx_downcast(1);
    PressureTimeseries.endprof(nb_downcast)=idx_downcast(end);
end
