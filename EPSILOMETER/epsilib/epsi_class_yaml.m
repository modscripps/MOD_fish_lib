classdef epsi_class_yaml < handle
    %
    ...
    %   Meta_Data_yaml_file = path to .yml file with Meta_Data values
    %%
    properties
        Meta_Data %read from yaml file
        filedir
        plot_properties
        epsi
        ctd
    ...
    %%
    methods
        function obj=epsi_class_yaml(varargin)

            if nargin<1
                data_path = pwd;
                Meta_Data_yaml_file = []; %Default Meta_Data process file not specified
            elseif nargin==1 %varargin is either data directory or Meta_Data_yaml_file
                if exist(varargin{1},'dir') %if varargin is a directory
                    data_path = varargin{1};
                    Meta_Data_yaml_file = [];
                elseif exist(varargin{1},'file') %if it's a file
                    Meta_Data_yaml_file = varargin{1};
                    data_path = pwd;
                end

            elseif nargin==2 %varagin is data directory and Meta_Data yaml file
                if exist(varargin{1},'dir') %if the first input is a directory
                    data_path = varargin{1};
                    Meta_Data_yaml_file = varargin{2};
            ...
            % If Meta_Data yaml file is specified, add it to epsi.Meta_Data.paths and read it
            % 
            if exist('Meta_Data_yaml_file','var')
            ...
            % if isempty(Meta_Data_yaml_file)  && ~isclassfield(Meta_Data.PROCESS,'filename')
            %     Meta_Data_yaml_file = fullfile(obj.Meta_Data.paths.process_library,'Meta_Data_Process','Meta_Data_Process.txt');
            % elseif isclassfield(Meta_Data.PROCESS,'filename')
            %     [~,filename,suffix] = fileparts(obj.Meta_Data.PROCESS.filename);
            %     Meta_Data_yaml_file = fullfile(obj.Meta_Data.paths.process_library,'Meta_Data_Process',[filename,suffix]);
            % end
            % obj.f_read_MetaProcess(Meta_Data_yaml_file);
...
%%
        function obj=f_read_MetaYaml(obj,filename)
            obj.Meta_Data = MODsetup_make_metadata_from_yaml(filename);
        end
%%
...
%%
        ...
        %             %                 tscanDefault = 50;
        %             %                 tscan =
        %             min([tscanDefault,tscanLongestProfile]);
        % 
