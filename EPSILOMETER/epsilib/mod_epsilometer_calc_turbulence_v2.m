...
%% Check that dTdV calibration has been set for this profile. If it has
% not, calculated it and save it in Profile.Meta_Data and big Meta_Data as
% one of the indices in an array


if max(size(Meta_Data.AFE.t1.cal))==1
    % If Meta_Data came from the Profile or a .mat file, there will only be one
    % cal value
    if Meta_Data.AFE.t1.cal==0 && Meta_Data.AFE.t2.cal==0
        Meta_Data=mod_epsi_temperature_spectra_v4(Meta_Data,Profile);
    end

elseif max(size(Meta_Data.AFE.t1.cal))>1
    %If Meta_Data came from top level of processing directory, there should
    %be one cal value per profile
    if Meta_Data.AFE.t1.cal(profNum)==0 && Meta_Data.AFE.t2(profNum).cal==0
        Meta_Data=mod_epsi_temperature_spectra_v4(Meta_Data,Profile);
    end
end
...
if sum(inRange)==0
    ...
    if saveData && isfield(Profile,'profNum')
        % Save Meta_Data with new t1 and t2 cals in Profile
        Profile.Meta_Data = Meta_Data;
        ...
        % Save big Meta_Data with t1 and t2 cals in the index for the current
        % profile
        Meta_Data.AFE.t1.cal(profNum) = Meta_Data.AFE.t1.cal;
        Meta_Data.AFE.t2.cal(profNum) = Meta_Data.AFE.t2.cal;
