...
%
% TODO:
%   If you're running in simulator mode, use the Setup+MDP data file in the
%   directory
% -------------------------------------------------------------------------
%
%% USER INPUTS
...
epsi_depth_array = 0:2000;
fctd_depth_array = 0:2000;
...
switch data_mode
    ...
    case 'simulator'
        ...
        % Path the processed data root directory
    ...
    case 'realtime'
        ...
        % Path to processed data root directory
...
% First, get list of existing MD_*.txt files
existing_files = dir(fullfile(input_struct.process_dir,'MD_*.txt'));
...
% Check if the new content is identical to any of the existing files
is_duplicate = false;
for i=1:length(existing_files)
    existing_file_path = fullfile(input_struct.process_dir,existing_files(i).name);
    fid = fopen(existing_file_path,'r');
    existing_content = fread(fid,'*char');
    fclose(fid)

    if isequal(existing_content,new_content)
        is_duplicate = true;
        input_struct.Meta_Data_process_file = existing_file_path;
        break;
    end
end

% If the content is new, make a file in the process directory called 'MD'
