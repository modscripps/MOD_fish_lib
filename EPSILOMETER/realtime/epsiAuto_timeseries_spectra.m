% October 2024 - This version adapted from epsiAuto_realtime_spectra.m to
% work with data files that have all the config and calibration info in the
% header.
%
...
% Summer 2021
% October 2024 - Updated to read config data from the .modraw file headers
% now that each header includes that info.
...
dirs.raw_incoming = input_struct.raw_dir;
...
obj = epsi_class(input_struct.raw_dir,input_struct.Meta_Data_process_file);
...
% as new data come in.
field_list = {'epsi','ctd','alt','vnav','gps','ttv','isap'};
