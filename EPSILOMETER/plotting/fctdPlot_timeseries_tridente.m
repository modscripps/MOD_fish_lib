function [ax] = fctdPlot_timeseries_tridente(obj,saveFig,ax)
...
function [ax] = setup_figure()
    ...
    ax(1)=subtightplot(7,1,1,gap,margV,margH); %T
    ax(2)=subtightplot(7,1,2,gap,margV,margH); %S
    ax(3)=subtightplot(7,1,3,gap,margV,margH); %fluor
    ax(4)=subtightplot(7,1,4,gap,margV,margH); %microconductivity
    accAx=subtightplot(7,1,5,gap,margV,margH); %divide the acc plots in two
