function [k,Pxx] = panchev_array(epsilon,kvis) 
% panchev
...
kmax=1./(5*eta);
dk=(kmax-k0)/200;
kin=arrayfun(@(x,y) (x:dk:y),k0,kmax,'un',0);

%  do the computation:
scale=2*pi*(epsilon*kvis^5).^0.25;
sum=cellfun(@(x) zeros(size(x)),kin,'un',0);
kn=cellfun(@(x,y) x/y,kin,num2cell(conv),'un',0);
