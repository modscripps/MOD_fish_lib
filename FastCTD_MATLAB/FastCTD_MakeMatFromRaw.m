...
for i=1:length(myASCIIfiles)
    ...
    if (~isempty(myMATfile) && (myASCIIfiles(i).datenum)>(myMATfile.datenum-1/24/60)) % added 1 minute or so for correcting time difference between computers
        fprintf(1,'Retranslating %s%s\n',MatDir,myMATfile.name);
        try
            disp([RawDir myASCIIfiles(i).name]);
%             FCTD = FastCTD_ReadASCII_YEI([RawDir myASCIIfiles(i).name]);
