...
for i = 1:length(fNames)
    if ~isfield(FCTD1,fNames{i}) || isempty(FCTD1.(fNames{i}))
        if ~isfield(FCTD2,fNames{i}) || isempty(FCTD2.(fNames{i}))
            FCTD.(fNames{i}) = [];
        else
    ...
    elseif isnumeric(FCTD1.(fNames{i}))
        if length(FCTD1.(fNames{i})) > 1 
            if isfield(FCTD2,fNames{i})
                FCTD.(fNames{i}) = [FCTD1.(fNames{i}); FCTD2.(fNames{i})];
            else % if you don't find the data for that field for some reason, add nans the size of FCTD2.time
                try % try to add an Nx1 nan array
                    FCTD.(fNames{i}) = [FCTD1.(fNames{i}); nan(size(FCTD2.time,1),size(FCTD2.time,2))];
                catch % try to add an NxM nan array
                    FCTD.(fNames{i}) = [FCTD1.(fNames{i}); nan(size(FCTD2.time,1),size(FCTD1.(fNames{i}),2))];
                end
            end
        elseif ~isempty(FCTD2.(fNames{i})) || isfield(FCTD2,fNames{i})
                FCTD.(fNames{i}) = [FCTD1.(fNames{i}); FCTD2.(fNames{i})];
        end
    else
        FCTD.(fNames{i}) = [];
