function out = GV_PlotUpAccumulation(matDataDir, rotDataDir, inputTimestamp)
...
% matDataDir = '/Users/gunnar/Projects/motive/cruises/cruise2/fctd-twist-counting/mat/';
% rotDataDir = '/Users/gunnar/Projects/motive/cruises/cruise2/fctd-twist-counting/rot/';
...
for iFile = 1:numel(matFiles)
    ...
    load(matFile,'vnav','ctd');

    % Only generate a rot file if missing or incomplete. 2 if exists, 0 if
    % not.
    rotFileExists = exist(rotFile, "file");

    if rotFileExists
        existing_rot = load(rotFile);
        if existing_rot.time(1) == vnav.dnum(1) && existing_rot.time(end) == vnav.dnum(end)
            rot_ok = true;
        else
            rot_ok = false;
        end
    ...
    if exist('vnav','var') && exist('ctd','var') && ...
        ...
        if rotFileExists
            existing_rot = load(rotFile);
            if existing_rot.time(1) == rot.dnum(1) && existing_rot.time(end) == rot.dnum(end)
                rot_ok = true;
            else
                rot_ok = false;
            end
        ...
        if ~rotFileExists || ~rot_ok
            display(['processing' rotFile])
...
if ~isempty(tot_time)
    ...
    % Return data
    out.time = datetime(tot_time,'ConvertFrom','datenum');
    out.rotation = gyro_value;
    out.pressure = tot_pressure;
