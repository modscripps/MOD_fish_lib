...
FastCTD_GUI_data.matDir = '/Users/Shared/EPSI_PROCESSING/Processed/FCTDmat';
...
FastCTD_GUI_Timer.Period = 60*10;
...
% NC - save FastCTD_GUI inside .matDir
save(fullfile(FastCTD_GUI_data.matDir,'FastCTD_GUI.mat'),'FastCTD_GUI_data');
...
% varNames = {'Backscatter','Temperature','Salinity','log($N^2\cdot[$s$^2])$'};
% varUnits = {'rfu', '$^\circ$C', 'psu',''};
varNames = {'Density','Temperature','Salinity','log($N^2\cdot[$s$^2])$'};
varUnits = {'kg/m$^3$', '$^\circ$C', 'psu',''};
...
for i = 1:4
   ...
   if i~=2
       set(handles.(sprintf('CB%1d',i)),'tickDir','in','xtick',[],'yaxislocation','right','FontName','Times','FontSize',12,'ydir','reverse');
   else
...
% If FastCTD_GUI_data is empty, create it from the default values   
elseif isempty(FastCTD_GUI_data)
    % NC - removed option to load whatever version of FastCTD_GUI it finds
    % - if it doesn't have it, go straight to creating it from default
    % values
...
myFCTD_GridData = [];
if isstruct(FCTD)
    if(isfield(FCTD,'eco_triplet'))
        FCTD.bs=FCTD.eco_triplet(:,1);
        FCTD.chla=FCTD.eco_triplet(:,2);
        FCTD.cdom=FCTD.eco_triplet(:,3);
        
        %smooth the chla and bs a bit to beat down some noise
        
        %keyboard
        chla=medfilt1(FCTD.chla,16,'omitnan');
        chlaN=naninterp(chla);
        % chlaN=naninterp(FCTD.chla);
        % chlaN(1)=chlaN(2);
        ...
        bs=medfilt1(FCTD.bs,16,'omitnan');
        bsN=naninterp(bs);
        % bsN=naninterp(FCTD.bs);
        % bsN(1)=bsN(2);
        % bsN(end)=bsN(end-1);
        % [b,a]=butter(6,0.5/8); %2 second low pass
        ...
        cdom=medfilt1(FCTD.cdom,16,'omitnan');
        ...
        % This is where the data are gridded onto two different grids, the regular and the tGrid
        % FastCTD_GridDataF grids data with a fluorometer
        myFCTD_GridData =  FastCTD_GridDataF(FCTD,'zMin',FastCTD_GUI_data.settings.DepthMin{3},'zMax',FastCTD_GUI_data.settings.DepthMax{3},'zInterval',1,'downcast');
    else
        % FastCTD_GridData grids data without a fluorometer
    ...
    % NC - add save data 
    save(fullfile(FastCTD_GUI_data.matDir,'FastCTD_data'),'myFCTD_GridData','FCTD');
end
% Color Plots
if isstruct(myFCTD_GridData) && size(myFCTD_GridData.tGrid.density,2)>2
    ...
    if(isfield(FCTD,'eco_triplet'))
        isopycnals = linspace(10,40,5000);
    else
    ...
    for i = 1
        if(i == 1 && ~isfield(FCTD,'eco_triplet'))
            continue;
        end
%         hold(handles.(sprintf('Frame%1d',i)),'off');
        ...
        data = myFCTD_GridData.tGrid.bs(:,time_ind);
...
for i = 1:length(ind);
    ...
    try
        ...
        % Try loading the first file since TimeMin
        try
...
%         keyboard;
        for j=1:numel(err.stack)
            disp([err.stack(j).file ' : ' err.stack(j).name ' : ' num2str(err.stack(j).line)]);
        end
    end
end
