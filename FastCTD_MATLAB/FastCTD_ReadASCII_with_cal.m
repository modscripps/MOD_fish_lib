function [FCTD] = FastCTD_ReadASCII_with_cal(fname,ctd_cal)
...
if linel>=ctdpos+ctdlen+mcrlen+cntlen
    ...
    if linel>=ctdpos+ctdlen+mcrlen+acclen+cntlen
        FCTD.VNAV_ASCII = s(:,ctdpos+ctdlen+mcrlen+(1:acclen)-1);
...
bad = ~ismember(FCTD.VNAV_ASCII,['0':'9' 'A':'F']);
FCTD.VNAV_ASCII(bad) = '0';
...
if isfield(FCTD,'ctd_ASCII')
    if isempty(FCTD.ctd_ASCII)
        ...
        FCTD = FastCTD_ASCII_getTemp(FCTD,ctd_cal);
        FCTD = FastCTD_ASCII_getPres(FCTD,ctd_cal);
        FCTD = FastCTD_ASCII_getCond(FCTD,ctd_cal);
    end
...
if isfield(FCTD,'mcr_ASCII')
    ...
    if ~isempty(FCTD.uConductivity)
        FCTD.eco_triplet = FCTD.uConductivity(:,1:3);
    else
        FCTD.eco_triplet = ones(0,3);
    end
    FCTD = rmfield(FCTD,'mcr_ASCII');
end

if isfield(FCTD,'VNAV_ASCII')
    if isempty(FCTD.VNAV_ASCII)
        FCTD.gyro = ones(0,3);
        FCTD.acceleration = ones(0,3);
        FCTD.compass = ones(0,3);
    else
        FCTD = FastCTD_ASCII_getVNAV(FCTD);
    end
    FCTD = rmfield(FCTD,'VNAV_ASCII');
end
...
unix(['grep INGGA  ' fname ' >> tmp.gps.txt']);
unix(['grep PHGGA  ' fname ' >> tmp.gps.txt']);
% local function...
...
function FCTD = FastCTD_ASCII_getVNAV(FCTD)
try
    m = reshape(FCTD.VNAV_ASCII',4,[])';
catch
    m = ones(0,4);
end
mm = hex2dec(m);
...
FCTD.gyro = tmp(:,7:9)/936;
FCTD.acceleration = tmp(:,4:6)/2047/9.8;
FCTD.compass = tmp(:,1:3)/13106;

% //with the vnav have mag range at +/- 2.5Gauss
% 	// acc range at +/- 16 g
% 	// gyro range at +/- 35 rad/s
% 	// the multiplier that would work to give maximum
% 	// precision are determined below
% 	static const float mag_mult = 13106.0;
% 	static const float acc_mult = 2047.0;
% 	static const float gyr_mult = 936.0;
...
try
    ...
    ind = FCTD.uConductivity(:,1) == 0 & FCTD.uConductivity(:,1)==FCTD.uConductivity(:,2) & FCTD.uConductivity(:,1) == FCTD.uConductivity(:,3);
    FCTD.uConductivity(ind,1:3) = NaN;
catch
    FCTD.uConductivity = ones(0,10);
end
...
function FCTD = FastCTD_ASCII_getTemp(FCTD,ctd_cal)
...
function FCTD = FastCTD_ASCII_getCond(FCTD,ctd_cal)
...
function FCTD = FastCTD_ASCII_getPres(FCTD,ctd_cal)

pa0 = ctd_cal.pa0;
pa1 = ctd_cal.pa1;
pa2 = ctd_cal.pa2;
ptempa0 = ctd_cal.ptempa0;
ptempa1 = ctd_cal.ptempa1;
ptempa2 = ctd_cal.ptempa2;
ptca0 = ctd_cal.ptca0;
ptca1 = ctd_cal.ptca1;
ptca2 = ctd_cal.ptca2;
ptcb0 = ctd_cal.ptcb0;
...
try
    ...
    for i = 1:length(line_length)
        if (line_length(i) >= 79)&&(line_length(i) <= 150)
